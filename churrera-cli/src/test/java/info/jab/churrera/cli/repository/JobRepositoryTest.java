...
class JobRepositoryTest {
    ...
    void shouldSaveAndFindJob() throws BaseXException, IOException, QueryException {
        ...
        Job job = new Job("test-job-1", "/path/to/job", null, "default-model", "default-repo", AgentState.UNKNOWN, now, now, null, null, null);
    ...
    void shouldFindAllJobs() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job job1 = new Job("job-1", "/path/1", null, "model1", "repo1", AgentState.UNKNOWN, now, now, null, null, null);
        Job job2 = new Job("job-2", "/path/2", null, "model2", "repo2", AgentState.UNKNOWN, now, now, null, null, null);
    ...
    void shouldDeleteJob() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job job = new Job("job-to-delete", "/path/to/delete", null, "model", "repo", AgentState.UNKNOWN, now, now, null, null, null);
        jobRepository.save(job);
    ...
    void shouldFindJobWithDetails() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job job = new Job("job-1", "/path/1", "cursor-agent-123", "model1", "repo1", AgentState.FINISHED, now, now, null, null, null);
    ...
    void shouldFindUnfinishedJobs() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job finishedJob = new Job("job-1", "/path/1", "cursor-agent-123", "model1", "repo1", AgentState.FINISHED, now, now, null, null, null);
        Job unfinishedJob1 = new Job("job-2", "/path/2", null, "model2", "repo2", AgentState.UNKNOWN, now, now, null, null, null);
        Job unfinishedJob2 = new Job("job-3", "/path/3", "cursor-agent-456", "model3", "repo3", AgentState.CREATING, now, now, null, null, null);
    ...
    void shouldUpdateExistingJob() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job originalJob = new Job("job-1", "/path/1", null, "model1", "repo1", AgentState.UNKNOWN, now, now, null, null, null);
    ...
    void shouldHandleJobWithNullCursorAgentId() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job jobWithNullAgent = new Job("job-1", "/path/1", null, "model1", "repo1", AgentState.UNKNOWN, now, now, null, null, null);
    ...
    void shouldHandleJobWithCursorAgentId() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job jobWithAgent = new Job("job-1", "/path/1", "cursor-agent-123", "model1", "repo1", AgentState.FINISHED, now, now, null, null, null);
    ...
    void shouldHandleAllAgentStates() throws BaseXException, IOException, QueryException {
        ...
        Job[] jobs = {
            ...
            new Job("job-2", "/path/2", "agent-2", "model2", "repo2", AgentState.CREATING, now, now, null, null, null),
            new Job("job-3", "/path/3", "agent-3", "model3", "repo3", AgentState.RUNNING, now, now, null, null, null),
            new Job("job-4", "/path/4", "agent-4", "model4", "repo4", AgentState.FINISHED, now, now, null, null, null),
            new Job("job-5", "/path/5", "agent-5", "model5", "repo5", AgentState.FAILED, now, now, null, null, null),
            new Job("job-6", "/path/6", "agent-6", "model6", "repo6", AgentState.FINISHED, now, now, null, null, null)
        };
    ...
    void shouldFindJobsByParentId() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job parentJob = new Job("parent-job", "/path/parent", "cursor-agent-123", "model1", "repo1", AgentState.FINISHED, now, now, null, null, null);
        Job childJob1 = new Job("child-job-1", "/path/child1", "cursor-agent-124", "model1", "repo1", AgentState.RUNNING, now, now, "parent-job", null, null);
        Job childJob2 = new Job("child-job-2", "/path/child2", "cursor-agent-125", "model1", "repo1", AgentState.CREATING, now, now, "parent-job", null, null);
        Job otherJob = new Job("other-job", "/path/other", "cursor-agent-126", "model1", "repo1", AgentState.FINISHED, now, now, null, null, null);
    ...
    void shouldHandleJobWithParentJobId() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job jobWithParent = new Job("child-job", "/path/child", "cursor-agent-123", "model1", "repo1", AgentState.RUNNING, now, now, "parent-job-id", null, null);
    ...
    void shouldHandleJobWithResult() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job jobWithResult = new Job("job-with-result", "/path/job", "cursor-agent-123", "model1", "repo1", AgentState.FINISHED, now, now, null, "Some result data", null);
    ...
    void shouldHandleJobWithWorkflowType() throws BaseXException, IOException, QueryException {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job sequenceJob = new Job("sequence-job", "/path/seq", "cursor-agent-123", "model1", "repo1", AgentState.FINISHED, now, now, null, null, info.jab.churrera.workflow.WorkflowType.SEQUENCE);
        Job parallelJob = new Job("parallel-job", "/path/par", "cursor-agent-124", "model1", "repo1", AgentState.FINISHED, now, now, null, null, info.jab.churrera.workflow.WorkflowType.PARALLEL);
    ...
    void shouldHandleInitializeWhenDatabaseExists() throws BaseXException, IOException, QueryException {
        ...
        LocalDateTime now = LocalDateTime.now();
        Job job = new Job("test-job", "/path/to/job", null, "model", "repo", AgentState.UNKNOWN, now, now, null, null, null);
        jobRepository.save(job);
        jobRepository.close();
    ...
    void shouldHandleMalformedJobXmlGracefully() throws BaseXException, IOException, QueryException {
        ...
        // Given
        LocalDateTime now = LocalDateTime.now();
        Job validJob = new Job("valid-job", "/path", null, "model", "repo", AgentState.UNKNOWN, now, now, null, null, null);
    ...
    void shouldHandleNullInEscapeXml() throws BaseXException, IOException, QueryException {
        ...
        LocalDateTime now = LocalDateTime.now();
        Job jobWithNulls = new Job("job-1", "/path", null, "model", "repo", AgentState.UNKNOWN, now, now, null, null, null);
    ...
    void shouldHandleJobWithInvalidWorkflowType() throws BaseXException, IOException, QueryException {
        ...
        Job job = new Job("job-1", "/path", "cursor-agent-123", "model1", "repo1",
                         ...
                         info.jab.churrera.workflow.WorkflowType.SEQUENCE);
    ...
    void shouldFindAllJobsWhenDatabaseContainsInternalReference() throws BaseXException, IOException, QueryException {
        ...
        for (int i = 0; i < 5; i++) {
            Job job = new Job("job-" + i, "/path/" + i, null, "model", "repo",
                            AgentState.UNKNOWN, now, now, null, null, null);
            jobRepository.save(job);
        }
    ...
    void shouldHandleRepositoryWithDefaultDatabasePath(@TempDir Path testTempDir) throws IOException, BaseXException, QueryException {
        ...
        // When
        LocalDateTime now = LocalDateTime.now();
        Job job = new Job("test-default", "/path", null, "model", "repo", AgentState.UNKNOWN, now, now, null, null, null);
